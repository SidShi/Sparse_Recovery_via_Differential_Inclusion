...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	in_y = mxGetPr(prhs[3]);
	in_category = mxGetScalar(prhs[4]);
	in_kappa = mxGetScalar(prhs[5]);
	in_alpha = mxGetScalar(prhs[6]);
	in_alpha0_rate = mxGetScalar(prhs[7]);
	in_result_r = mxGetPr(prhs[8]);
	in_group_split = mxGetPr(prhs[9]);
	in_group_split_length = mxGetScalar(prhs[10]);
	in_intercept = mxGetScalar(prhs[11]);
	in_tlist = mxGetPr(prhs[12]);
	in_nt = mxGetScalar(prhs[13]);
	in_trate = mxGetScalar(prhs[14]);
	in_print = mxGetScalar(prhs[15]);


	
	LB_multi_logistic(in_X, &in_row, &in_col, in_y, &in_category, &in_kappa, &in_alpha, &in_alpha0_rate, in_result_r, in_group_split,
